...
import { EXDB, EXIDX } from "@/domain/exercises/exercises.js";
import { uid } from "@/shared/lib/format.js";
import type { SetFields } from "@/domain/training/history.js";
import type {
  BodyweightEntry,
  CustomEx,
  Id,
...
/** A parsed timestamp: iso date plus optional time-of-day in epoch-ms offsets. */
export interface When {
  d: IsoDate;
  t: number | null;
}

export type WorkoutsImport =
  | { error: "empty" | "unrecognised" }
  | {
      kind: "workouts";
      source: string | null;
      workouts: Workout[];
...
export type ParseResult = WorkoutsImport | WeightsImport;
...
const SYN: ReadonlyArray<readonly [RegExp, string]> = [
  [/\bbb\b/g, "barbell"],
  [/\bdb\b/g, "dumbbell"],
  [/\bkb\b/g, "kettlebell"],
...
// Rows in flight carry the row's own weight unit (`u`) into the conversion pass below;
// it never reaches the stored set.
type RowSet = SetFields & { done: boolean; u?: "" | Unit };

// a cardio set carries neither field; the original read them as undefined → 0
const fields = (s: LoggedSet): SetFields => s;

/**
 * Read an export into workouts Set & Signal understands, WITHOUT touching state — the caller
...
export function parseWorkoutCSV(
  ...
  // "185 lb" into 185 kg.
  const fileUnit: "" | Unit = sawLb && !sawKg ? "lb" : sawKg && !sawLb ? "kg" : "";
...
export function parseBodyweight(
  ...
  const s = String(text);
  const out = new Map<IsoDate, { w: number; t: number | null }>(); // iso date -> weigh-in (one per day, the last)
  let fileUnit: "" | Unit = "";
...
export function mergeImport(
  state: Pick<
    import("@/shared/lib/types.js").AppState,
  ...
  if ("kind" in parsed && parsed.kind === "workouts") {
    ...
    state.customEx = [...(state.customEx || []), ...customs];
    state.workouts = [...state.workouts, ...fresh].sort((a, b) => (a.d < b.d ? -1 : 1));
