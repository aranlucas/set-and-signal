...
import { isoOf } from "@/shared/lib/format";
import type { RepsSet, Workout } from "@/shared/lib/types";
...
const W = (
  ...
  return {
    ...
    d: isoOf(d),
    start: +d,
    // fields the effort code never reads, filled to satisfy the Workout contract
    end: +d,
    routineId: null,
    name: "",
    prs: [],
    vol: 0,
    // fixtures only vary the optional rating fields; w/r/done are always present
    entries: [
      {
        id: "0025",
        topW: null,
...
describe("displayScale", () => {
  ...
  it("shows an unrated profile the scale its imported history is written in", () => {
    ...
    expect(
      displayScale({
        effort: "none",
        ...profileWithWorkouts(W(3, [{ rpe: 8 }, { rpe: 9 }])),
      }),
    ).toBe("rpe");
    expect(
      displayScale({
        effort: "none",
        ...profileWithWorkouts(W(3, [{ rir: 2 }])),
      }),
    ).toBe("rir");
...
describe("effortSummary", () => {
  ...
  it("leaves unrated sets out of the average instead of reading them as failure", () => {
    const r = effortSummary(profileWithWorkouts(W(2, [{ rir: 4 }, {}, {}, {}, {}, {}])), 0);
  ...
  it("survives a profile with no training at all", () => {
    expect(effortSummary({ workouts: [] }, 30)).toEqual({
      done: 0,
      rated: 0,
      hard: 0,
      avg: null,
...
describe("hasEffort", () => {
  it("decides whether the effort UI exists at all", () => {
    expect(hasEffort(profileWithWorkouts(W(2, [{ rir: 0 }])))).toBe(true);
    expect(hasEffort(profileWithWorkouts(W(2, [{ rpe: 8 }])))).toBe(true);
    expect(hasEffort(profileWithWorkouts(W(2, [{}, { rir: null }])))).toBe(false);
    expect(hasEffort({ workouts: [] })).toBe(false);
  });

  it("ignores sets that were never finished", () => {
    expect(hasEffort(profileWithWorkouts(W(2, [{ rir: 2, done: false }])))).toBe(false);
  });
});

describe("effortWeeks", () => {
  it("averages per calendar week and carries the week volume alongside", () => {
    const pts = effortWeeks(profileWithWorkouts(W(1, [{ rir: 1 }, { rir: 3 }, {}])), 0);
  ...
  it("drops a week that rests on a single tap", () => {
    expect(effortWeeks(profileWithWorkouts(W(1, [{ rir: 1 }])), 0)).toEqual([]);
  });

  it("comes back oldest first, whatever order the workouts arrived in", () => {
    const pts = effortWeeks(
...
describe("effortHistogram", () => {
  ...
  it("is all zeroes, not NaN, when nothing is rated", () => {
    const h = effortHistogram(profileWithWorkouts(W(2, [{}])), 0);
