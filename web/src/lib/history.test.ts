...
describe("capEffort", () => {
  ...
  it("passes an emptied field through untouched", () => {
    ...
    expect(capEffort("rpe")).toBe(undefined);
...
describe("defaultConfig", () => {
  it("gives each mode a sensible starting point", () => {
    expect(defaultConfig(LIFT)).toEqual({
      sets: 3,
      reps: 10,
      weight: 0,
      mode: "reps",
    ...
    expect(defaultConfig(LIFT, "time")).toEqual({
      sets: 3,
      sec: 45,
      weight: 0,
      mode: "time",
  ...
  it("seeds the bodyweight flag from the catalogue, and only when it is true", () => {
    expect(defaultConfig(BW)).toEqual({
      sets: 3,
      reps: 10,
      weight: 0,
      mode: "reps",
    ...
    expect(defaultConfig(BW, "time")).toEqual({
      sets: 3,
      sec: 45,
      weight: 0,
      mode: "time",
  ...
  it("starts a barbell exercise at the supplied empty-bar weight", () => {
    expect(defaultConfig(BARBELL, "reps", 45).weight).toBe(45);
    expect(defaultConfig(BARBELL, "time", 20).weight).toBe(20);
  });
});
...
describe("exLine", () => {
  ...
  it("summarises a planned exercise per mode", () => {
    ...
    expect(exLine({ id: LIFT, sets: 2, sec: 90, weight: 20, mode: "time" }, "kg")).toBe(
      "2 × 1:30 · 20 kg",
    );
...
describe("buildSets", () => {
  ...
  it("uses the configured bar as the minimum starting load for barbell work", () => {
    const state = {
      workouts: [],
      exWeights: {},
      unit: "lb" as const,
  ...
  it("builds cardio sets unchanged", () => {
    expect(buildSets(emptyS, { id: CARDIO, sets: 1, min: 25, speed: 9 })).toEqual([
  ...
  it("does not seed a duration from a rep count when an exercise switches to time", () => {
    const state = {
      ...
      workouts: [
        {
          d: "2026-01-01",
          entries: [{ id: LIFT, sets: [{ w: 60, r: 10, done: true }] }],
        },
      ],
    };
